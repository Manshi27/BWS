...
for path, subdirnames, filenames in os.walk("trainingImages"):
    for filename in filenames:
        ...
        result_array=np.append(result_array,[result.astype(np.float),datalist[0],dob_list[0],dob_list[1]])
        print(result_array[0])
        array_set=np.append(array,[result_array[0].astype(np.float),(float)(count)])
